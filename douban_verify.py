...
driver.find_element_by_xpath('//*[@id="username"]').send_keys('15182623583')
driver.find_element_by_xpath('//*[@id="password"]').send_keys('15182623583.gmy')
driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div[5]/a').click()
time.sleep(1)
# 4、切换到新的iframe子页面 - 滑块验证
iframe_node = driver.find_element_by_xpath('//*[@id="TCaptcha"]/iframe')
driver.switch_to.frame(iframe_node)
# 5、按住开始滑动位置按钮 - 先移动180个像素
node = driver.find_element_by_xpath('//*[@id="slideBlock"]')
# 用鼠标点击改节点且保持
...
# 6、使用加速度函数移动剩下的距离
tracks = get_tracks(20)  # tranks为列表,代表每一个0.3秒对应的位移
for item in tracks:
    ActionChains(driver).move_by_offset(xoffset=item, yoffset=0).perform()  # 鼠标从当前位置移动的距离
# 7、延迟释放鼠标: release()移动
time.sleep(0.5)
